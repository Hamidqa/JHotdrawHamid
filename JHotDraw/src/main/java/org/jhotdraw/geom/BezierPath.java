...
//Klassen kaldes fra DATANOde
    public void checkNode(DataNode dataNode) {
        if (dataNode.getX() < dataNode.getX1()) {
            dataNode.setX1(dataNode.getX());
        }
        if (dataNode.getY() < dataNode.getY1()) {
            dataNode.setY1(dataNode.getY());
        }
        if (dataNode.getX() > dataNode.getX2()) {
            dataNode.setX2(dataNode.getX());
        }
        if (dataNode.getY() > dataNode.getY2()) {
            dataNode.setY2(dataNode.getY());
        }
    }
    ...
    public Rectangle2D.Double getBounds2D() {
        if (bounds == null) {
            ...
            if (size == 0) {             
                ...
                // handle first node
                Node node = get(0);
                node2.setY1(node.y[0]);
                node2.setY2(node.y[0]);
                node2.setX1(node.x[0]);
                node2.setX2(node.x[0]);
                if (isClosed && (node.mask & C1_MASK) != 0) {
                    node2.setY(node.y[1]);
                    node2.setX1(node.x[1]);
                    checkNode(node2);
                }
            ...
            bounds = new Rectangle2D.Double(node2.getX1(), 
                    node2.getY1(), node2.getX2() - node2.getX1(), 
                    node2.getY2() - node2.getY1());
        }
